...
void RunState::update(float delta_time, Entity* target, Map* map) {
	...
	float stepX = entity->getSpeedValue() * 500 * signX * delta_time;
	float stepY = entity->getSpeedValue() * 500 * signY * delta_time;
	...
	if (adjucentLeg <= abs(stepX + entity->getHitboxShape()->getSize().x * signX)) {
		...
		if (entity->calcDistance(target->getObjectPosition() + target->getHitboxPosition()) < 100) {
			entity->setState(new WalkState(entity));
			return;
		}
		else if (entity->calcDistance(target->getObjectPosition() + target->getHitboxPosition()) > 300) {
			entity->setState(new IdleState(entity));
			return;
		}
		return;
